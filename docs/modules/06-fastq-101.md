...
```bash
mkdir -p ~/bioinfo-course/module11 && cd ~/bioinfo-course/module11
cat > tiny.fq << 'EOF'
@r1
ACGTTGCA
+
...
=== "Linux/WSL"
    ...
    ```bash
    zcat tiny.fq.gz | awk -v N=1000 'NR%4==2{
      seq=$0; gc_seq=seq; gsub(/[^GgCc]/,"",gc_seq)
      gc+=length(gc_seq); bp+=length(seq); n++
      if(n==N) exit
    } END{ if(bp==0) print 0; else printf("%.2f\n", 100*gc/bp) }'
...
=== "macOS"
    ...
    ```bash
    gzcat tiny.fq.gz | awk -v N=1000 'NR%4==2{
      seq=$0; gc_seq=seq; gsub(/[^GgCc]/,"",gc_seq)
      gc+=length(gc_seq); bp+=length(seq); n++
      if(n==N) exit
    } END{ if(bp==0) print 0; else printf("%.2f\n", 100*gc/bp) }'
